		...
		$.ajax({
			...
			success: function(html){
				if(html=='true'){
					alert("Message sent!");
					alert(counter);
					showdetail(id,isbn,(_counter+1),1);
				}
				else {
...
function showdetail(_partner,_isbn,_counter,identifier){
	
	counter=""+_counter;
	alert(identifier);
	
	if(identifier)
		readTag(_partner, _isbn);
...
function addBook(isbn,title,author,picture,category,list) {
	...
	if(list == 1){
		...
		while(price ==""||(price!=null)){
			//alert(price);
			if(isUnsignedInteger(price))
	...
	$.ajax({
		type: "POST",
		url:"addbook.php",
		data:"isbn="+isbn+"&title="+escape(title)+"&author="+author+"&picture="+escape(picture)+"&category="+category+"&list="+list+"&price="+price,
		success: function(html){
			if(html=='true'){
				//var e=jQuery.Event("click");
				//jQuery(".close-reveal-modal").trigger(e);
...
function bookInfoPopup(isbn, title, author, picture,id, price){
	
	alert(price);
	if(price){
			var popup_data = "<img src='"+picture+"' class = 'bookimg' /><h3>"+title+"</h3><p>By "+author+"</p><p>ISBN: "+isbn+"</p><p>Price: "+price+" dollars</p><a class='close-reveal-modal'>&#215</a><input type='button' value='remove from list' onClick='removeBook("+id+","+isbn+","+1+")'/>";
	}
	else{
		var popup_data = "<img src='"+picture+"' class = 'bookimg' /><h3>"+title+"</h3><p>By "+author+"</p><p>ISBN: "+isbn+"</p><a class='close-reveal-modal'>&#215</a><input type='button' value='remove from list' onClick='removeBook("+id+","+isbn+","+1+")'/>";
	}
	
	$("#generic_test").html(popup_data);
